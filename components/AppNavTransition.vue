<template>
  <transition-group tag="div">
    ...
    <button @click="toggleFollow" :class="[following ? followActiveClass : '', followDefaultClass]" key="follow">
    ...
    <div @click="addPlace" class="side-icon" key="sideicon">
      ...
      <icon-base v-else icon-name="plus" icon-color="white" width="18" height="18" class="plus">
        <icon-plus />
      </icon-base>
    </div>
...
export default {
  ...
  data () {
    return {
      following: false,
      followDefaultClass: 'follow',
      followActiveClass: 'active-follow',
      activeUser: 'profile-photo-primary',
  ...
  methods: {
    changeUser (i) {
      this.$store.commit('CHANGE_USER', i);
      if (this.page === 'group') {
    ...
    toggleFollow () {
      if (this.following) {
        this.$store.commit('REMOVE_FOLLOWER');
      } else {
    ...
    addAnimation () {
      ...
      tl.add('start')
      tl.to('.plus', 0.75, { rotation: -360, transformOrigin: '50% 50%', ease: Expo.easeOut }, 'start')
      tl.to('.line2', 0.7, { scaleY: 0.5, x: -2, rotation: -45, transformOrigin: '50% 100%', ease: Expo.easeOut }, 'start')
      tl.to('.line1', 0.7, { rotation: -50, x: 7, scaleX: 3, transformOrigin: '50% 100%', ease: Expo.easeOut }, 'start')
      tl.fromTo('.saveinfo', 0.5, { autoAlpha: 0 }, { autoAlpha: 1, ease: Sine.easeOut }, 'start')
      tl.to('.saveinfo', 0.4, { autoAlpha: 0, ease: Expo.easeIn }, 'start+=1')
    ...
    removeAnimation () {
      const tl = new TimelineMax()
      tl.add('begin')
      tl.to('.plus', 0.75, { rotation: 0, transformOrigin: '50% 50%', ease: Expo.easeOut }, 'begin')
      tl.to('.line2', 0.7, { scaleY: 1, x: 0, rotation: 0, transformOrigin: '50% 100%', ease: Expo.easeOut }, 'begin')
      tl.to('.line1', 0.7, { rotation: 0, x: 0, scaleX: 1, transformOrigin: '50% 100%', ease: Back.easeOut }, 'begin')
      tl.timeScale(1.2)
...
.index .profile-photo-primary.profile-0 {
